...
class Gui(QWidget):
    ...
    def get_usb_devices(self):
        ...
        if platform.system() == 'Linux':
            for device in os.listdir('/dev/disk/by-label'):
                if device != "ESP" and device != "EFI":
                    devices.append([device, os.path.realpath('/dev/disk/by-label/'+device)])
    ...
    def eject_usb(self, usb):
        ...
        if confirm == QMessageBox.Yes:
            # make a message box to type in the password
            if password_dialog(self) == None:
                password_dialog(self)
            

            password, ok = QInputDialog.getText(self, 'Password', 'Enter your password:')
            # it does not seem to work, "no password provided" error. to fix this we need to use a different method
            

            if ok:
                if platform.system() == 'Linux':
                    os.system('echo '+password+' | sudo -S umount '+usb[1])
                elif platform.system() == 'Windows':
